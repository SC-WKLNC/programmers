...
public class 기능개발_권영찬 {
    ...
    public static int[] solution(int[] progresses, int[] speeds) {
        ...
        for(int i=0; i<progresses.length; i++) { // for 루프
            ...
            if(dDay >= (int)tmp) { // 우선순위가 높은 기능이 남은 일수가 더 많거나 같을 경우 배포 기능 갯수 +1
                if(dDay == 100) dDay = (int)tmp; // 첫번째 루프에 낮은 우선순위 남은 일수로 D-Day 초기화
                fCnt[dpCnt]++;
            }
            else { // 아닐경우 배포 횟수 +1,  배포 기능 갯수 +1, 낮은 우선순위 남은 일수로 D-Day 초기화
                fCnt[++dpCnt]++;
                dDay = (int)tmp;
            }
        }
